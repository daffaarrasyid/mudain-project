...
@section('content')
<!-- Tambahkan state untuk Alpine.js: id, nama, dll -->
<div x-data="{ 
        modalTambah: false, 
        modalEdit: false, 
        modalHapus: false,
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 mb-3">Kategori</h2>
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
            <thead>
                <tr class="text-gray-800 text-sm font-bold border-b-2 border-gray-100">
                    <th class="px-6 py-4 w-3/4">Kategori</th>
            ...
            <tbody class="text-sm text-gray-600" id="tableBody">
                ...
                <!-- Tambahkan class data-row untuk target search JS -->
                <tr class="data-row border-b border-gray-50 even:bg-gray-50/70 hover:bg-orange-50/40 transition-colors duration-200 animate-fade-in-up delay-{{ $index * 100 }}">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col gap-1.5 items-center justify-center">
                            <!-- Inject data ke Alpine state saat diklik -->
                            <button @click="modalEdit = true; editId = '{{ $kat->id }}'; editNama = '{{ $kat->nama_kategori }}'" class="bg-[#38BDF8] hover:bg-[#0284C7] text-white px-4 py-1 rounded w-20 text-xs font-semibold shadow-sm transition-colors">
                                Edit
                            </button>
                            <!-- Inject id ke Alpine state saat diklik -->
                            <button @click="modalHapus = true; hapusId = '{{ $kat->id }}'" class="bg-[#EF4444] hover:bg-[#B91C1C] text-white px-4 py-1 rounded w-20 text-xs font-semibold shadow-sm transition-colors">
                                Hapus
                            </button>
                        </div>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="2" class="px-6 py-8 text-center text-gray-400">Belum ada data kategori.</td>
                </tr>
                @endforelse
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mt-8 text-sm text-gray-500 gap-4">
        <!-- Menampilkan X sampai Y dari Z Data (Sesuai request lu) -->
        <div>
            Menampilkan <span class="font-bold text-gray-700">{{ $kategoris->firstItem() ?? 0 }}</span> 
            sampai <span class="font-bold text-gray-700">{{ $kategoris->lastItem() ?? 0 }}</span> 
            dari <span class="font-bold text-[#E65C00]">{{ $kategoris->total() }}</span> data
        </div>
        ...
        <div class="flex items-center gap-2 text-sm">
            ...
            @else
                <a href="{{ $kategoris->previousPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-left text-xs"></i></a>
            @endif

            <!-- (Sebagai contoh desain lu, gue tampilin statis angka 1) Nanti lo bisa custom loop buat multiple pages jika perlu -->
            <button class="w-8 h-8 rounded-full flex items-center justify-center bg-[#E65C00] text-white font-bold shadow-md">{{ $kategoris->currentPage() }}</button>

            <!-- Tombol Next -->
            @if($kategoris->hasMorePages())
                <a href="{{ $kategoris->nextPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-right text-xs"></i></a>
            @else
    ...
    <!-- MODAL TAMBAH (Backend Connected) -->
    <div x-show="modalTambah" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalTambah = false"
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
                <h3 class="text-xl font-bold text-gray-800">Tambah Data Kategori</h3>
            ...
            <form action="{{ route('admin.kategori.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama_kategori" placeholder="Contoh: Sablon, Bordir..." required
    ...
    <div x-show="modalEdit" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalEdit = false"
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
                <h3 class="text-xl font-bold text-gray-800">Edit Data Kategori</h3>
            ...
            <!-- Action form menyesuaikan dengan editId -->
            <form :action="`{{ url('admin/master-data/kategori-produk') }}/${editId}`" method="POST" class="space-y-4">
                @csrf
                @method('PUT') 
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
                    <!-- x-model untuk mengisi value dari baris yang diklik -->
    ...
    <div x-show="modalHapus" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-center m-auto" @click.away="modalHapus = false"
            ...
            <p class="text-sm text-gray-500 mb-6">Anda yakin ingin menghapus kategori ini? Data yang terkait mungkin akan terpengaruh.</p>
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-3">
                ...
                <!-- Action form menyesuaikan dengan hapusId -->
                <form :action="`{{ url('admin/master-data/kategori-produk') }}/${hapusId}`" method="POST" class="w-full sm:w-auto">
                    @csrf
                    @method('DELETE') 
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function(e) {
            ...
            tableRows.forEach(row => {
                // Mengambil seluruh teks di dalam baris (Nama Kategori)
