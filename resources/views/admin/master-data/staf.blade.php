...
<div x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 mb-3">Staf Servis</h2>
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
            <thead>
                <tr class="text-gray-800 text-sm font-bold border-b-2 border-gray-100">
                    <th class="px-6 py-4 w-1/5">Kode Staf</th>
                    <th class="px-6 py-4 w-2/5">Nama Pegawai</th>
                    <th class="px-6 py-4 w-1/5">Keahlian Jasa</th>
                    <th class="px-6 py-4 text-center w-1/5">Opsi</th>
                </tr>
            </thead>
            <tbody class="text-sm text-gray-600" id="tableBody">
                ...
                <tr class="data-row border-b border-gray-50 even:bg-gray-50/70 hover:bg-orange-50/40 transition-colors duration-200 animate-fade-in-up delay-{{ $index * 100 }}">
                    <td class="px-6 py-4 font-bold text-[#E65C00]">{{ $staf->kode_staf }}</td>
                    <td class="px-6 py-4 font-medium text-gray-700">
                        {{ $staf->nama_staf }}<br>
                        <span class="text-xs text-gray-400 font-normal"><i class="fa-solid fa-phone mr-1"></i> {{ $staf->no_telp ?? 'Tidak ada no. telp' }}</span>
                    </td>
                    <td class="px-6 py-4">
                        @if($staf->servis->nama_servis == 'Fotografer')
                            <span class="bg-green-50 text-green-600 px-3 py-1 rounded-lg text-xs font-bold">{{ $staf->servis->nama_servis }}</span>
                        @elseif($staf->servis->nama_servis == 'Videografer')
                            <span class="bg-blue-50 text-blue-600 px-3 py-1 rounded-lg text-xs font-bold">{{ $staf->servis->nama_servis }}</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col gap-1.5 items-center justify-center">
                            <button @click="modalEdit = true; editId = '{{ $staf->id }}'; editNama = '{{ $staf->nama_staf }}'; editTelp = '{{ $staf->no_telp }}'; editServis = '{{ $staf->servis_id }}'" class="bg-[#38BDF8] hover:bg-[#0284C7] text-white px-4 py-1 rounded w-20 text-xs font-semibold shadow-sm transition-colors">
                                Edit
                            </button>
                            <button @click="modalHapus = true; hapusId = '{{ $staf->id }}'" class="bg-[#EF4444] hover:bg-[#B91C1C] text-white px-4 py-1 rounded w-20 text-xs font-semibold shadow-sm transition-colors">
                                Hapus
                            </button>
                        </div>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center text-gray-400">Belum ada data pegawai.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mt-8 text-sm text-gray-500 gap-4">
        <div>
            Menampilkan <span class="font-bold text-gray-700">{{ $stafs->firstItem() ?? 0 }}</span> 
            sampai <span class="font-bold text-gray-700">{{ $stafs->lastItem() ?? 0 }}</span> 
            dari <span class="font-bold text-[#E65C00]">{{ $stafs->total() }}</span> data
        </div>
        <div class="flex items-center gap-2 text-sm">
            ...
            @else
                <a href="{{ $stafs->previousPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-left text-xs"></i></a>
            @endif
            <button class="w-8 h-8 rounded-full flex items-center justify-center bg-[#E65C00] text-white font-bold shadow-md">{{ $stafs->currentPage() }}</button>
            @if($stafs->hasMorePages())
                <a href="{{ $stafs->nextPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-right text-xs"></i></a>
            @else
    ...
    <div x-show="modalTambah" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalTambah = false"
            ...
            <form action="{{ route('admin.staf.store') }}" method="POST" class="space-y-4">
                @csrf
                <input type="hidden" name="kode_staf" value="{{ $newCode }}">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pegawai <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_staf" placeholder="Contoh: Budi Santoso" required
                ...
                <div>
                    ...
                    <input type="text" name="no_telp" placeholder="0812345..." required
                           class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Keahlian (Jasa) <span class="text-red-500">*</span></label>
                    <select name="servis_id" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                        <option value="">Pilih Jasa</option>
                        @foreach($dataServis as $ds)
                            <option value="{{ $ds->id }}">{{ $ds->nama_servis }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div x-show="modalEdit" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalEdit = false"
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
                <h3 class="text-xl font-bold text-gray-800">Edit Data Pegawai</h3>
            ...
            <form :action="`{{ url('admin/master-data/staf') }}/${editId}`" method="POST" class="space-y-4">
                @csrf @method('PUT') 
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pegawai <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_staf" x-model="editNama" required
                           class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">No. Telepon <span class="text-red-500">*</span></label>
                    <input type="text" name="no_telp" x-model="editTelp" required
                           class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Keahlian (Jasa) <span class="text-red-500">*</span></label>
                    <select name="servis_id" x-model="editServis" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                        @foreach($dataServis as $ds)
                            <option value="{{ $ds->id }}">{{ $ds->nama_servis }}</option>
                        @endforeach
                    </select>
    ...
    <div x-show="modalHapus" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-center m-auto" @click.away="modalHapus = false"
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">Hapus Pegawai?</h3>
            <p class="text-sm text-gray-500 mb-6">Anda yakin ingin menghapus data pegawai ini?</p>
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-3">
                ...
                <form :action="`{{ url('admin/master-data/staf') }}/${hapusId}`" method="POST" class="w-full sm:w-auto">
                    @csrf @method('DELETE') 
