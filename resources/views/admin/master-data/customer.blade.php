...
<!-- State Alpine.js untuk menampung semua interaksi Modal & Data Edit -->
<div x-data="{ 
        modalTambah: false, 
        modalEdit: false, 
        modalHapus: false,
        modalImport: false,
        editId: '', editKode: '', editNama: '', editTelp: '', editEmail: '', editJenis: '', editAlamat: '',
        hapusId: ''
    }" 
    ...
    <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 mb-3">Customer</h2>
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('admin.customer.export-pdf') }}" class="bg-[#38BDF8] hover:bg-[#0284C7] text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-lg shadow-blue-500/30 transition-all transform hover:-translate-y-0.5">
                    Export PDF
                </a>
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody class="text-sm text-gray-600" id="tableBody">
                ...
                <tr class="data-row border-b border-gray-50 even:bg-gray-50/70 hover:bg-orange-50/40 transition-colors duration-200 animate-fade-in-up delay-{{ $index * 100 }}">
                    <td class="px-6 py-4 font-bold text-[#E65C00]">{{ $cus->kode_customer }}</td>
                    <td class="px-6 py-4 font-medium text-gray-700">{{ $cus->nama_customer }}</td>
                    <td class="px-6 py-4">{{ $cus->no_telp }}</td>
                    <td class="px-6 py-4">{{ $cus->email ?? '-' }}</td>
                    <td class="px-6 py-4 truncate max-w-[150px]">{{ $cus->alamat ?? '-' }}</td>
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 rounded-full text-xs font-semibold
                            {{ $cus->jenis_customer == 'Pelanggan' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600' }}">
                            {{ $cus->jenis_customer }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col gap-1.5 items-center justify-center">
                            <!-- Parsing data ke state edit -->
                            <button @click="modalEdit = true; editId = '{{ $cus->id }}'; editKode = '{{ $cus->kode_customer }}'; editNama = '{{ $cus->nama_customer }}'; editTelp = '{{ $cus->no_telp }}'; editEmail = '{{ $cus->email }}'; editAlamat = '{{ $cus->alamat }}'; editJenis = '{{ $cus->jenis_customer }}'" class="bg-[#38BDF8] hover:bg-[#0284C7] text-white px-4 py-1 rounded w-16 text-xs font-semibold shadow-sm transition-colors">
                                Edit
                            </button>
                            <button @click="modalHapus = true; hapusId = '{{ $cus->id }}'" class="bg-[#EF4444] hover:bg-[#B91C1C] text-white px-4 py-1 rounded w-16 text-xs font-semibold shadow-sm transition-colors">
                                Hapus
                            </button>
                        </div>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-6 py-8 text-center text-gray-400">Belum ada data customer.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between mt-8 text-sm text-gray-500 gap-4">
        <div>
            Menampilkan <span class="font-bold text-gray-700">{{ $customers->firstItem() ?? 0 }}</span> 
            sampai <span class="font-bold text-gray-700">{{ $customers->lastItem() ?? 0 }}</span> 
            dari <span class="font-bold text-[#E65C00]">{{ $customers->total() }}</span> data
        </div>
        <div class="flex items-center gap-2 text-sm">
            ...
            @else
                <a href="{{ $customers->previousPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-left text-xs"></i></a>
            @endif

            <button class="w-8 h-8 rounded-full flex items-center justify-center bg-[#E65C00] text-white font-bold shadow-md">{{ $customers->currentPage() }}</button>

            @if($customers->hasMorePages())
                <a href="{{ $customers->nextPageUrl() }}" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-500 bg-white border border-gray-200 hover:bg-[#E65C00] hover:text-white transition-colors shadow-sm"><i class="fa-solid fa-chevron-right text-xs"></i></a>
            @else
    ...
    <div x-show="modalTambah" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto overflow-x-hidden p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalTambah = false"
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
                <h3 class="text-xl font-bold text-gray-800">Tambah Data Customer</h3>
            ...
            <form action="{{ route('admin.customer.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <!-- Auto Generate via Controller -->
                        <input type="text" name="kode_customer" value="{{ $newCode }}" class="w-full bg-gray-100 border border-gray-200 rounded-lg px-4 py-2.5 text-gray-500 font-bold cursor-not-allowed" readonly>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Customer <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_customer" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Customer <span class="text-red-500">*</span></label>
                        <select name="jenis_customer" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
    ...
    <div x-show="modalEdit" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto overflow-x-hidden p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-6 md:p-8 m-auto" @click.away="modalEdit = false"
            ...
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
                <h3 class="text-xl font-bold text-gray-800">Edit Data Customer</h3>
            ...
            <form :action="`/admin/master-data/customer/${editId}`" method="POST" class="space-y-4">
                @csrf
                @method('PUT') 
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Customer <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_customer" x-model="editNama" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">No. Telp / WA <span class="text-red-500">*</span></label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Customer <span class="text-red-500">*</span></label>
                        <!-- Select dropdown binding value using x-model -->
                        <select name="jenis_customer" x-model="editJenis" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-[#E65C00] focus:ring-1 focus:ring-[#E65C00]">
                            <option value="">Pilih Jenis Customer</option>
                            <option value="Umum">Umum</option>
                            <option value="Pelanggan">Pelanggan</option>
                        </select>
    ...
    <div x-show="modalHapus" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-center m-auto" @click.away="modalHapus = false"
            ...
            <p class="text-sm text-gray-500 mb-6">Yakin ingin menghapus data customer ini? Riwayat transaksi pelanggan ini mungkin akan terdampak.</p>
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-3">
                ...
                <form :action="`/admin/master-data/customer/${hapusId}`" method="POST" class="w-full sm:w-auto">
                    @csrf
                    @method('DELETE')
    ...
    <div x-show="modalImport" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl p-6 text-center m-auto" @click.away="modalImport = false"
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">Import Data Customer</h3>
            <p class="text-sm text-gray-500 mb-6">Pilih file Excel (xls/xlsx/csv) untuk mengimpor data massal.</p>
            
            <form action="{{ route('admin.customer.import-excel') }}" method="POST" enctype="multipart/form-data">
                @csrf
