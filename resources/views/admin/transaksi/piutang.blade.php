...
@section('content')
    <div class="space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-2xl font-bold text-slate-900">Data Piutang</h2>
            <p class="mt-2 text-sm text-slate-500">Pantau piutang penjualan, ubah status, dan catat penerimaan pembayarannya.</p>
        </div>

        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" class="grid gap-3 md:grid-cols-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari customer" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="grid gap-6 lg:grid-cols-[1.1fr_1fr_1fr]">
                        <div>
                            ...
                            <h3 class="mt-1 text-lg font-bold text-slate-900">{{ $item->id_penjualan }}</h3>
                            <p class="mt-2 text-sm text-slate-600">{{ $item->penjualan?->pembeli?->nama_pembeli ?? 'Umum' }}</p>
                            <p class="text-sm text-slate-500">Tanggal: {{ $item->tanggal->format('d M Y H:i') }}</p>
                            <p class="mt-3 text-sm text-slate-500">Total piutang: <span class="font-semibold text-slate-900">Rp {{ number_format($item->jumlah_piutang, 0, ',', '.') }}</span></p>
                            <p class="text-sm text-slate-500">Sisa: <span class="font-semibold text-amber-600">Rp {{ number_format($item->sisa, 0, ',', '.') }}</span></p>
                        </div>

                        <form action="{{ route('admin.transaksi.piutang.status', $item) }}" method="POST" class="space-y-3 rounded-2xl bg-slate-50 p-4">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{ route('admin.transaksi.piutang.bayar', $item) }}" method="POST" class="space-y-3 rounded-2xl bg-slate-50 p-4">
                            @csrf
                            <h4 class="font-semibold text-slate-900">Penerimaan Piutang</h4>
                            ...
                            <button type="submit" class="w-full rounded-2xl bg-orange-500 px-4 py-3 text-sm font-semibold text-white">Terima Pembayaran</button>
                        </form>
                    </div>
                </div>
            @empty
