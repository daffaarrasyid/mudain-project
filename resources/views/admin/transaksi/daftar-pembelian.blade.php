    ...
    <div class="space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Daftar Pembelian</h2>
                    <p class="mt-2 text-sm text-slate-500">Seluruh transaksi pembelian dari pemasok yang sudah masuk ke sistem.</p>
                </div>
                <a href="{{ route('admin.transaksi.entry-pembelian') }}" class="rounded-2xl bg-orange-500 px-4 py-3 text-sm font-semibold text-white">Tambah Pembelian</a>
            </div>
        </div>

        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" class="grid gap-3 md:grid-cols-4">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari invoice atau supplier" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($items as $item)
                            <tr>
                                <td class="px-4 py-4 font-semibold text-slate-900">{{ $item->id_pembelian }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $item->tanggal->format('d M Y H:i') }}</td>
                                <td class="px-4 py-4 text-slate-700">{{ $item->pemasok?->nama_pemasok }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-slate-500">Belum ada data pembelian.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
