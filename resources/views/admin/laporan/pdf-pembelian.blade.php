...
<body>
    ...
    <div class="title-block">
        <h2>Laporan Pembelian Barang (Kulakan)</h2>
        <span class="subtitle">
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="font-bold text-orange">{{ $trx->invoice ?? $trx->faktur }}</td>
                <td>{{ $trx->supplier->nama_supplier ?? 'Umum / Tanpa Nama' }}</td>
                <td>{{ $trx->user->name ?? 'Admin' }}</td>
                <td class="text-center {{ $trx->status_pembayaran == 'Lunas' ? 'text-green' : 'text-red' }}">
            ...
            @empty
            <tr>
                <td colspan="7" class="text-center" style="color: #999; padding: 20px;">Tidak ada data pembelian pada periode ini.</td>
            </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr style="background-color: #fff8f5; font-weight: bold;">
                <td colspan="6" class="text-right font-bold text-orange" style="font-size: 10px; border-top: 2px solid #E65C00;">TOTAL PENGELUARAN</td>
                <td class="text-right font-bold text-orange" style="font-size: 11px; border-top: 2px solid #E65C00;">Rp {{ number_format($totalPengeluaran, 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
